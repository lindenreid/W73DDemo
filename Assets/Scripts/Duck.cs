...
public class Duck : MonoBehaviour
{
    ...
    private void RunFacingCheck (Player player)
    {
        ...
        // prevents displaying dialogue if player is too far away
        bool isWithinDistance = Vector3.Distance(transform.position, _playerPos) <= _dialogueDistance;
        if(isWithinDistance)
        {
            // get vector pointing from the duck to the player
            _playerToMe = transform.position - _playerPos;
            _playerToMe = _playerToMe.normalized; // not necessary, but makes debug visualizations clearer
